...
class JobOfferFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        foreach (self::JOB_OFFERS as $index => $offerData) {
            ...
            $offer->setUpdatedAt(new DateTimeImmutable('now'));
            
            // Références aux autres entités
            $offer->setJobCategory($this->getReference('category_' . $offerData['category'], JobCategory::class));
            $offer->setJobType($this->getReference('job_type_' . $offerData['type'], TypeJob::class));
            ...
            $this->addReference('job_offer_' . ($index + 1), $offer);
        }
